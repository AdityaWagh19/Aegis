 ...
 * All figures are the API's all-time aggregates; per-batch rupee totals live
 * in the Batches view, where they actually exist.
 */
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    getMetrics()
      .then(setMetrics)
      .catch(() => setError('Could not reach the Aegis API. Start it with uvicorn on port 8000.'));
  }, []);
  ...
  return (
    <AppShell title="Overview">
      {error && (
        <div
          role="alert"
          className="rounded-md bg-danger-tint border border-danger/30 text-danger text-[13px] px-16 py-8"
        >
          {error}
        </div>
      )}
      ...
      {empty && (
        <div className="bg-pure-white rounded-lg border border-stone-border shadow-md p-48 flex flex-col items-center gap-16 text-center">
          <p className="font-roobert text-[18px] text-ink-black">No mandates processed yet</p>
      ...
      {metrics && metrics.total_records > 0 && (
        <>
          {/* All-time stats */}
          <section className="bg-pure-white rounded-lg border border-stone-border shadow-md p-24">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-24 mt-16">
              <Stat label="Mandates processed" value={String(metrics.total_records)} />
          ...
          <div className="grid lg:grid-cols-2 gap-24">
            ...
            <section className="bg-pure-white rounded-lg border border-stone-border shadow-md p-24 overflow-x-auto">
              <h2 className="text-caption leading-caption font-medium uppercase tracking-[0.025em] text-warm-gray mb-16">
...
function Stat({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="font-roobert text-[28px] leading-none text-ink-black tabular-nums">{value}</p>
      {context && (
        <p
          className={`text-[12px] ${
